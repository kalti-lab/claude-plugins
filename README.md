# kalti-lab-notes 플러그인

kalti 연구실용 스킬 묶음. Claude Code 플러그인 형식으로 배포하며, 이 형식을 지원하는 Codex에서도 같은 마켓플레이스에서 설치할 수 있다.

## kalti-lab-notes

연구일지 작성·지식 조회·주간 정리·온톨로지 정제 규약을 LLM에 주입하는 스킬 묶음.
...
터미널에서는 같은 작업을 CLI 명령으로 실행할 수 있다:

```bash
claude plugin marketplace add kalti-lab/claude-plugins
claude plugin install kalti-lab-notes@kalti-lab
```
...
```bash
codex plugin marketplace add kalti-lab/claude-plugins
codex plugin add kalti-lab-notes@kalti-lab
codex plugin list --marketplace kalti-lab
```

Codex의 설치 명령은 `install`이 아니라 `add`다. 마켓플레이스를 추가한 뒤에는 `codex plugin add` 대신 Codex CLI의 `/plugins`에서 설치할 수도 있다. Codex는 [Claude 호환 마켓플레이스 형식](https://learn.chatgpt.com/ko-KR/docs/enterprise/plugin-management)을 직접 읽으므로 같은 저장소를 사용한다.

#### 최초 실행과 호출

설치 또는 업데이트 후 새 Claude Code/Codex 세션을 시작한다. 스킬 호출 표기는 다음과 같다.
...
본인 폴더 안에서 일지는 **프로젝트별 하위 폴더**로 정리되고, 파일 이름은 제목뿐이다 — `journals/<본인>/<프로젝트>/제목.md`(프로젝트가 없으면 `_inbox/`). 날짜는 `date` 칸에 있고 이름에는 넣지 않는다. 대신 **이름이 볼트 전체에서 겹치지 않아야 한다** — 위키링크가 파일 이름으로만 대상을 찾기 때문이고, 검사 스크립트가 저장할 때마다 확인한다. `kalti-journal`은 호출될 때마다 아직 정리 안 된 파일을 해당 프로젝트 폴더로 옮긴다(이름이 바뀌면 그 파일을 가리키는 `[[ ]]` 링크도 함께 재작성).
...
- **`KALTI_GIT_SYNC`** — 작성·정제 후 git 동기화를 어디까지 할지. `ask`(기본, 쓸 때마다 푸시할지 물어봄) / `push`(자동 commit→pull rebase→push) / `commit`(로컬 커밋만) / `off`(git 안 건드림). 미설정은 `ask`로 취급. (이건 *스킬이 어디까지 시도하나*만 정함 — git 명령 실행 허가 자체는 Claude Code/Codex의 권한 설정이 따로 관장한다.)
...
Obsidian CLI 사용법과 Obsidian 문법(마크다운·Bases·Canvas)을 모델에 가르치는 **공식 스킬 묶음**(Obsidian CEO 제작). `kalti-setup`이 설치를 요구하는 필수 구성요소다.
