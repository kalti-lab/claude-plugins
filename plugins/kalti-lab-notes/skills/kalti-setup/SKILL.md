...
**아래 순서로만 정한다. 디스크를 광역으로 긁지 말 것** — `find ~/dev ...` 같은 추측 스캔은 금지다. 여기 명시 안 된 행동을 지어내지 않는다.

1. `notes.env`의 `$KALTI_VAULT`가 유효하면(그 안에 `journals/` 있음) 그걸로. 끝.
2. 아니면 **현재 폴더(또는 바로 위 한두 단계)**에 `journals/`+`ontology/`가 있는지만 본다 — 있으면 그게 볼트(클론 안에서 실행한 경우). 안 묻고 그대로.
3. 그래도 없으면 **반드시 AskUserQuestion으로 딱 두 가지만** 묻는다(다른 선택지·디스크 추측 금지):
   1. **현재 폴더에 셋팅** — 여기(`<pwd>`)에 `git clone https://github.com/kalti-lab/lab-notes.git`로 받아 볼트로 쓴다
   2. **지정 경로 있어?** — "기타(직접 입력)"로 경로를 받는다(이미 가진 볼트면 그 경로, 없으면 거기에 clone)
