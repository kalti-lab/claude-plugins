---
name: kalti-ontology
description: "kalti 연구일지를 온톨로지(지식 그래프)로 정제하는 운영자용 규약. journals/의 일지에서 가설·발견을 뽑아 ontology/에 6가지 객체(project·hypothesis·finding·concept·source·person)와 타입드 링크로 큐레이션한다. kalti 온톨로지 객체를 새로 만들거나, 일지를 정제·승격하거나, '온톨로지 정제'·'지식그래프 갱신'·'이번 주 일지 정제해줘'·'발견/가설 카드로 올려줘' 같은 요청 시 반드시 사용. 멤버의 일지 작성(kalti-journal 스킬)과 달리, 이건 운영자가 증거를 지식으로 끌어올리는 작업이다. 근거 없는 객체를 만들지 않도록 일지 원문 인용으로 검증하는 게 핵심."
---
...
- **일지 층 `journals/<이름>/`** — 멤버가 쓰는 작업 기록 = **증거**.
- **온톨로지 층 `ontology/`** — 일지에서 정제해 뽑은 "계속 살아있는 지식"을 객체로 만든 층. 그래프에 뜨는 의미 노드. **운영자만 쓴다.**

이 스킬은 일지를 **온톨로지로 정제(큐레이션)**하는 쪽이다. 멤버는 일지만 쓴다. 정제의 핵심은 "일지에 실제로 근거가 있는 결론만, 중복 없이" 객체로 끌어올리는 것 — 없는 사실을 만들면 그래프 전체의 신뢰가 깨진다.

## 어디서 작업하나 — 볼트 위치부터 잡기

전역 플러그인이라 **어느 디렉터리에서 호출되든** 동작하므로, 운영자 볼트(일지+온톨로지를 함께 받은 클론)의 루트를 **맨 먼저 확정**한다. 아래 `journals/`·`ontology/` 경로는 모두 이 루트(이하 `$VAULT`) 기준 절대경로다. 정하는 순서:
...
(`ontology/`는 운영자 볼트에만 있다 — 멤버 클론엔 없거나 `.gitignore`로 빠져 있다.)
...
멤버가 쌓은 일지를 온톨로지로 끌어올리는 작업. 모든 참여자가 일지로 기여하지만, **온톨로지 쓰기는 운영자 한 사람이 게이트**한다(AI 정제 게이트). 그래서 실수로 남의 지식이 깨지지 않는다.

1. **입력**: 그 주에 새로 쌓인 일지 + 멤버 `proposals/` 메모.
2. **AI 정제**: 일지에서 "아직 온톨로지에 없는 가설/발견 후보 + 링크"를 뽑게 한다(아래 프롬프트).
3. **운영자 승인**: 후보가 실제 일지에 근거하는지·중복 아닌지 확인하고 `ontology/`에 반영한다. 근거 없는 건 버린다.
4. **주기**: 주 1회(주간 보고서 만들 때 같이).
...
운영자는 통과한 후보를 읽고 승인된 것만 `ontology/`에 파일로 만든다(`references/object-templates.md`의 블록 사용).

## 왜 두 층인가

일지는 증거, 온톨로지는 지식이다. 그래프엔 온톨로지만 의미 노드로 뜨고 일지는 증거로 매달린다. 그래서 멤버 부담은 낮고(일지만 쓰면 됨), 그래프는 깔끔하며, 큐레이션을 거친 지식은 신뢰할 수 있다. 또 관련된 것만 링크를 따라 정밀하게 뽑아낼 수 있어, AI에 줄 컨텍스트가 작고 정확해진다.
