...
볼트를 잡았으면 **본인 폴더(`$AUTHOR`)도 확정**한다 — 추측하면 오타로 엉뚱한 새 폴더(`journals/Aram/` 등)를 파버린다:

1. **환경변수**: `echo "$KALTI_AUTHOR"`로 확인. 값이 있고 `$VAULT/journals/<값>/`이 실재하면 그걸 `$AUTHOR`로 쓴다.
2. 없으면 `$VAULT/journals/`의 **기존 폴더 목록**을 보여주고 **AskUserQuestion으로 고르게** 한다. 새 멤버면 "기타(직접 입력)"로 핸들(영문 소문자 권장)을 받아 `mkdir -p`로 폴더를 만든다.
3. 다음부터 안 묻게 셸 설정에 `export KALTI_AUTHOR=<핸들>`을 넣어두라고 안내한다 — 또는 `/kalti-setup`을 한 번 돌리면 볼트·폴더·CLI까지 한꺼번에 잡힌다.
