...
호출됐는데 **이번 세션에 일지로 남길 실제 작업이 없으면**(방금 켠 빈 세션, 직전 대화가 일지거리가 아님 등) **절대 내용을 창작하지 않는다.** 특히 `journals/`의 다른 일지(남의 폴더 포함)를 읽어 그걸 소재로 글을 지어내면 안 된다 — 그건 증거가 아니라 허구이고, 일지 시스템 전체의 신뢰를 깬다. 이럴 땐 쓰기를 멈추고 사용자에게 먼저 묻는다: **"이번에 무슨 작업을 일지로 남길까? 한 일·실행한 명령·결과를 알려줘."** 답을 받은 뒤에야 작성에 들어간다.

기존 일지를 읽어도 되는 건 딱 두 경우다 — ① 이어쓸 후보 파일을 **찾기 위해**(제목·`project`·날짜 등 **메타데이터만** 본다), ② 형식 참고(`assets/journal-template.md`·`references/example-experiment.md`). 다른 일지의 **내용**을 이번 일지의 소재로 끌어오지 않는다.

근거가 세션·사용자 설명에 있을 때만, 정말 빠진 메타데이터(어느 `project`인지, 검증한 가설 노트, `author` 이름)를 짧게 한 번 묻고 진행한다. **추론으로 메우지 않는다.**
...
- **근거(세션·사용자 설명) 없이 일지를 창작하지 않는다** — 특히 다른 사람의 일지를 읽어 소재로 삼지 않는다. 쓸 거리가 없으면 멈추고 먼저 묻는다(일지는 증거지 허구가 아니다).
