...
## 한 일 (방법)
어떻게·왜 그 방법이었는지 서술. 결과를 좌우하는 값·파라미터·버전은 수치 그대로 — 단, 변수·함수·파일 이름 자체는 적지 않는다.
코드 내부 이름(함수·필드·플래그·DB컬럼·env키)은 뜻으로 풀어 쓰고, 한 줄에 한 메시지로.
(커밋 해시·push 범위·수정파일 목록·함수명/줄번호·PID·단계 번호·diff/라인 수치·임시/scratchpad 경로는 넣지 않는다. 저장 전 "고도 점검" 참고 — SKILL.md.)
