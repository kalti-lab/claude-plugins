...
집계 문맥은 주지 않는다 — 격리가 목적이다.

이어서 **맨눈 검수**를 돌린다(korean-polish의 「발행물은 맨눈 검수를 한 번 더」).
규칙 문서를 주지 않은 검수자 둘에게 같은 HTML을 "처음 읽는 한국어 화자" 관점으로
보고만 받아, 겹치는 지적을 반영하고 나머지는 실행 요약에 싣는다 — 표 기반 패스가
"고칠 것 없음"을 낸 지면에서 맨눈 검수 둘이 겹치는 지적 십여 건을 찾은 실측
(2026-09-14)이 이 단계의 근거다.

두 검수가 끝나면 글자 수 상한(Step 3의 표)을 다시 세고, 새로 걸린 낱말은 실행
요약에 옮겨 적는다.
